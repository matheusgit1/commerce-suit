...
export class AuthApi {
  ...
  public async resentVerifyCode(
    body: IResentVerifyCode,
    headers?: AxiosRequestHeaders
  ) {
    const response = await this.httpClient.execute.post(
      "/users/validate/resent-verify-code",
      body,
      headers || {}
    );
    return response;
  ...
  public async resetPassword(
    ...
    const response = await this.httpClient.execute.put(
      "/users/resete-password",
      body,
      headers || {}
    );
    return response;
  ...
  public async changePasswordWithoutLogin(
    body: IResetPasswordWithoutLogin,
    headers?: AxiosRequestHeaders
  ) {
    const { token, ...rest } = body;
    const response = await this.httpClient.execute.put(
      `/users/resete-password/${body.token}`,
      {
        email: body.email,
