...
export default function MyPurchases({}) {
  ...
  React.useEffect(() => {
    const initialize = async () => {
      try {
        const { data } = await productContext.getUserCartInDetails(
    ...
    initialize();
  }, [pagination, productContext.cartIds]);
  ...
  const removeFromCart = async (productName: string, productId: string) => {
    try {
      const { data } = await productContext.removeFromCart(
        authContext.user?.access_token || "",
        productId
      );
      ...
      setPurchaseList(
        purchaseList.filter((values) => values.co_product_id != productId)
      );
    ...
    <React.Fragment>
      ...
      {purchaseList?.length === 0 && (
        <Result
          ...
          extra={
            <Button onClick={() => navigate(paths.home)} type="primary">
              Ir para o inico
            </Button>
          }
        />
                ...
                <Space>
                  <Button
                    ...
                    onClick={() => {
                      ...
                      // }
                      navigate(
                        `/minhas-compras/checkout/${values.co_product_id}`,
                        { state: { data: values } }
                      );
                    }}
                    style={{
                  ...
                  <Button
                    ...
                    onClick={() =>
                      removeFromCart(
                        values.co_product_name,
                        values.co_product_id
                      )
                    }
                    danger
                    icon={<DeleteOutlined />}
                  ...
                  <Meta
                    ...
                    avatar={
                      width > 600 && (
                        <img
                          ...
                          src={values?.co_product_images[0] || ""}
                        />
                      )
                    }
                    title={
                    ...
                    description={
                      <>
                        ...
                        <br />
                        <Text type="secondary">id de compra: {values.id}</Text>
                        <br />
                      </>
                    }
                  />
