...
import { IAdressFormat } from "../../services";
...
export default function Checkout({}) {
  ...
  //@ts-ignore
  const [productdata, setProductData] = React.useState<IUseProductCheckout>(
    location.state?.data
  );
  ...
  const [quantityInCart, setQuantityInCart] = React.useState<number>(
    productdata?.co_quantity || 0
  );
  ...
  const [modalData, setModalData] = React.useState<IAdressFormat | undefined>(
    authContext.userAdress
  );
  const [loading, setLoading] = React.useState<boolean>(false);
  const [typeModalAdress, setTypeModalAdres] =
    React.useState<TAdressForm>("edit");
  ...
  const onFinish = async (values: any) => {
    try {
      ...
      navigate(`/minhas-compras/checkout/${params.id}/finalizado`, {
  ...
  const updateCart = async () => {
    try {
      ...
      const response = await productContext.updateCart(
        productdata.co_product_id,
        quantityInCart,
        authContext.user?.access_token
      );
      ...
      setProductData({
        ...productdata,
        co_product_stocks: String(quantityInCart),
      });
          ...
          <Col span={18}>
            ...
            <Row style={{ justifyContent: "flex-end", marginTop: 15 }}>
              <Space>
                ...
                />
                <Button onClick={() => setActiveTab("adress")} type="primary">
                  ir para endereços
                </Button>
                <Button onClick={() => setActiveTab("payment")} type="primary">
                  finalizar compra
                </Button>
              </Space>
            </Row>
          ...
          >
            <Col span={18}>
              ...
              >
                ...
                >
                  você está adiquirindo {quantityInCart} unidades de{" "}
                  {productdata.co_product_name} ao custo de R${" "}
                  {+productdata.co_product_price * +quantityInCart} - preço
                  unitário R${+productdata.co_product_price}
                </Title>
              </Row>
  ...
  return (
    <React.Fragment>
      <Tabs
        onTabClick={(e: any) => setActiveTab(e)}
        activeKey={activeTab}
        draggable
        centered
      >
