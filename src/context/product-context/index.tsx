...
export type ProductContextType = {
  ...
  listPurchase: (pagination: number, token?: string) => AxiosPromise;
  wishList: string[];
  cartIds: string[];
};
...
export function ProductContextProvider(props: ProductContextProvidersProps) {
  ...
  React.useEffect(() => {
    const initialize = async () => {
      ...
      try {
        const { data } = await purchaseApi.listWishlist(
  ...
  React.useEffect(() => {
    const initialize = async () => {
      if (!authContext.user) return;
      try {
        const { data } = await purchaseApi.getUserCart(
          authContext.user?.access_token || ""
        );
        for (const index in data) {
  ...
  const getListProductByCategory = async (body: IFilter) => {
  ...
  const removeFromWishlist = async (productId: string, token: string) => {
    ...
    setWishList(
      wishList.filter((productIdContent) => productIdContent !== productId)
    );
    return response;
  };
  ...
  const getUserWishlistInDetails = async (
    token: string,
    pagination: number
  ) => {
    const response = await purchaseApi.getUserWishlistInDetails(
      token,
      pagination
    );
    return response;
  ...
  const listPurchase = async (pagination: number, token?: string) => {
    const response = await purchaseApi.listPurchase(pagination, token);
    return response;
  };
