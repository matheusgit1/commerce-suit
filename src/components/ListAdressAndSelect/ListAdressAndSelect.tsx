...
export const ListAdressAndSelect: React.FC<props> = ({
  ...
  const deleteAdress = async (adressId: string) => {
    try {
      setLoading(true);
      const { status } = await authContext.deleteUserAdress(
        { adressId: adressId },
  ...
  return (
    <React.Fragment>
      ...
      <Modal
        title={`${
          typeModalAdress == "edit" ? "Editando" : "Criando endereço"
        } ${typeModalAdress == "edit" ? modalData?.street : ""}`}
        centered
        ...
        authContext.userListAdress?.map((values, index) => (
          <Card
            style={{
              marginTop: 8,
              boxShadow:
                values.id === authContext.userAdress?.id
                  ? "rgba(26, 222, 4, 0.979) 0px 3px 8px"
              ...
              <Col>
                {width > 400 && (
                  <Button
                    ...
                    style={{
                      color:
                        authContext.userAdress?.id === values.id
                          ? "white"
                          : "blue",
                      background:
                        authContext.userAdress?.id === values.id
                ...
                <Button
                  ...
                  icon={<PlusCircleOutlined />}
                  style={{
                    visibility: buttonAddNewAdress ? "visible" : "hidden",
                  }}
                >
                  Adicionar
                </Button>
            ...
            >
              <Meta
                ...
                description={
                  <>
                    <Space>
                      <Text type="secondary">Cidade: {values.city}</Text>
                      <Text type="secondary">Número: {values.number}</Text>
                      <Text type="secondary">Bairro: {values.district}</Text>
                      <Text type="secondary">
                        Quadra: {values.block || "Não definido"}
                      </Text>
                      <Text type="secondary">UF: {values.uf}</Text>
                    </Space>
                    {/* <Text type="secondary">
                      Id: {values.id}
                    </Text><br /> */}
